...
class Robot:
    ...
    def next_state(
        self, current_state, joint_and_wheel_velocities, delta_t, max_joint_and_wheel_velocity
    ):
    ...
    def TrajectoryGenerator(self):
        return trajectory_generatator.TrajectoryGenerator(
            self.Tse_initial, self.Tsc_initial, self.Tsc_final, self.Tce_grasp, self.Tce_standoff, self.k
        )   